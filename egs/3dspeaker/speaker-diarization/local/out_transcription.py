...
def main(args):
    ...
    # Save diarization_result.json if --output is provided
    if args.output:
        diar_json = []
        for rec_id in rec_ids:
            rttm_path = os.path.join(sys_rttm_dir, rec_id+'.rttm')
        ...
        with open(args.output, "w") as f:
            json.dump({"segments": diar_json}, f, indent=2)
        print(f"✅ diarization_result.json saved to {args.output}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--exp_dir', type=str, default="", help="exp dir")
    parser.add_argument('--gpu', nargs='+', help='GPU id to use.')
    parser.add_argument('--output', type=str, default=None, help="Optional output path for diarization_result.json")
